...
import {addFromItems} from '/first_try/Scripts/cartAdd.js';
...
window.addEventListener('load', () => {
    let dataId = localStorage.getItem('item')
    let filteredFrom1 = CatalogList.filter(item => item.id == dataId);
    let filteredFrom2 = _MoreAboutItems.filter(item => item.id == dataId);
    if (dataId > 29 && dataId < 36) {
